"""Pure encode for Family N3 rasters.

encode(px, width, height) accepts only raster bytes and dimensions.
It does not import a generator and does not read a seed.
Decode redraws the stored program. No residual, no correction bytes.

Fitter constants are copied from experiments/paint_list_n2/FROZEN_CONSTANTS.txt
and are not changed.
"""
...
VERSION = 1
SETTINGS_CODE = 1
SETTINGS_ID = "paint-list-n3-v1"
MAX_SHAPES = 6
MAX_PALETTE = 8
# Copied from experiments/paint_list_n2/FROZEN_CONSTANTS.txt
# before Family N3 held-out images were generated. Not swept. Not retuned.
ELLIPSE_MARGIN = 4
TRI_LO = -2
TRI_HI = 6
LINE_RADIUS = 8
...
def split_two_rects(pts):
    """Partition a merged same-color mask into two filled rectangles.

    Two non-overlapping axis-aligned rectangles are separable by a vertical
    or horizontal line. Shared-edge pairs are one 4-connected component, so
    a single-primitive fit fails and this split is required for exactness.
    Returns two (TYPE_RECT, geom) or None. Not a residual.
    """
    if not pts or len(pts) < 2:
        return None
    s = set(pts)
    x0, y0, x1, y1 = _bbox(pts)
    if len(s) == (x1 - x0 + 1) * (y1 - y0 + 1):
        return None

    def pack(part):
        bx0, by0, bx1, by1 = _bbox(part)
        return (TYPE_RECT, (bx0, by0, bx1, by1))

    for split_x in range(x0, x1):
        left = []
        right = []
        for p in pts:
            if p[0] <= split_x:
        ...
        if not left or not right:
            continue
        if _is_filled_rect(left) and _is_filled_rect(right):
            return [pack(left), pack(right)]

    for split_y in range(y0, y1):
        top = []
        bot = []
        for p in pts:
            if p[1] <= split_y:
        ...
        if not top or not bot:
            continue
        if _is_filled_rect(top) and _is_filled_rect(bot):
            return [pack(top), pack(bot)]
    return None


def fit_color_components(pts, width, height):
    """Fit each 8-connected component. Shared-edge rects are split, not merged."""
    fitted = []
