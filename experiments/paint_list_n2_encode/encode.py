"""Pure encode for Family N2 committed rasters.

encode(px, width, height) accepts only raster bytes and dimensions.
It does not import a generator, does not read a seed, and does not take
generator arguments. Decode redraws the stored program. No residual,
no correction bytes.
...
ELLIPSE_MARGIN = 4
# Outward local offsets copied with the frozen fitter. Not changed.
...
def _fit_degenerate_line(pts, bits, width, height):
    """Collinear filled triangles rasterize as a line. Hull has fewer than 3 points.
