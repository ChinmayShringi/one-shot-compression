"""Family N2 image-only ordered paint-list codec. No correction bytes.

The encoder sees pixels only. It recovers a palette and an ordered list of
filled shapes (type, geometry, color index). Colors may be shared by more than
one shape. Decode redraws that program with the shared rasterizer. There is no
XOR plane, residual, or extra correction field. If no such program redraws the
image exactly, encode returns None and the image is not stored.
...
pixel set is split into 8-connected components. Each component must be exactly
one filled rect, ellipse, or triangle. Shapes of the accepted program are
stored in a deterministic recovered order. Overlap that clips a shape so a
component is not one primitive fails exactness.

Fitter constants were copied from experiments/paint_list/codec.py at
e1af093fe560908b2a9a8c9939de4436462ea57e before held-out images were generated.
They are not swept on held-out seeds.
"""
...
VERSION = 1
SETTINGS_CODE = 1
SETTINGS_ID = "paint-list-n2-v1"
MAX_SHAPES = 6
MAX_PALETTE = 8
# Copied from experiments/paint_list/codec.py at e1af093fe560908b2a9a8c9939de4436462ea57e
# before Family N2 held-out images were generated. Not swept on held-out seeds.
ELLIPSE_MARGIN = 4
...
def scene_from_program(payload):
    """Unpack a stored program without redrawing. Used by the checker for reuse counts."""
    if payload[:4] != MAGIC or payload[4] != VERSION:
        raise ValueError("bad paint-list payload")
    if payload[5] != SETTINGS_CODE:
        raise ValueError("bad settings code")
...
def program_reuses_color(payload):
    scene = scene_from_program(payload)
    counts = {}
...
def connected_components(pts):
    """8-connected components, discovered in scan order (y, then x)."""
    if not pts:
        return []
    remaining = set(pts)
    comps = []
    for start in sorted(remaining, key=lambda p: (p[1], p[0])):
...
def fit_color_components(pts, width, height):
    """Fit each 8-connected component of one color. None if any component fails."""
    fitted = []
    for comp in connected_components(pts):
        one = fit_color_mask(comp, width, height)
        if one is None:
            return None
        fitted.append(one)
    return fitted
...
def recover_program(px, width, height):
    ...
    if not colors or len(colors) > MAX_PALETTE:
        return None
    fitted_by_color = [fit_color_components(pts, width, height) for pts in buckets]
    accepted = None
    for bg_index, bg in enumerate(colors):
        ...
        for cidx, color in enumerate(colors):
            ...
            if not fitted:
                ok = False
                break
            for typ, geom in fitted:
                shapes.append((typ, color, geom))
